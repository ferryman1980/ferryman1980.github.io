...
# 访问 http://localhost:3001 进入管理控制台
...
# 创建租户管理员
curl -X POST http://localhost:3001/api/tenants/<tenant_id>/users \
  -H "Authorization: Bearer <admin_token>" \
  -H "Content-Type: application/json" \
  -d '{
...
server {
    ...
    location / {
        proxy_pass http://localhost:3001/tenants/$tenant_id;
...
```
http://your-logto-domain/callback/{connectorId}
```
...
- **logto 官网**: [https://github.com/logto-io/logto](https://github.com/logto-io/logto)
